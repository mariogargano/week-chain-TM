...
export default async function WeekBalancePage() {
  ...
  // Fetch WEEK balances
  const { data: balances } = await supabase?.from("week_balances")?.select(`
      *,
      users (
        email,
        full_name
  ...
  const { data: transactions } = await supabase?.from("week_transactions")?.select(`
      *,
      users (
        email,
        full_name
                ...
                {balances?.map((balance) => (
                  <tr key={balance?.id} className="border-b hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4 font-semibold text-green-600">
                      {(Number.parseFloat(balance?.balance) - Number.parseFloat(balance?.locked_balance))?.toLocaleString()}{" "}
                      WEEK
                    </td>
