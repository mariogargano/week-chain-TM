...
export async function GET() {
  try {
    ...
    const { data: certificates } = await supabase?.from("user_certificates")?.select("tier_id, weeks_per_year, start_date, end_date")?.eq("status", "active")

    // Get total supply capacity
    const { data: supplyProps } = await supabase?.from("supply_properties")?.select("total_weeks_capacity")?.eq("status", "active")
